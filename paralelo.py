...
def envio_mail(mail, nombre):
    return requests.post(
        ...
        files=[("attachment", open("Envio/"+nombre+".pdf")),
                           ("attachment", open("Envio/"+nombre+".fasta"))],
        data={"from": "Cluster Marvel<postmaster@sandbox3761d267c8184fa2afb562ff83f8881e.mailgun.org>",
              ...
              "\nUn PDF con información general ("+nombre+".pdf) y un archivo FASTA con el detalle de las alineaciones ("+nombre+".fasta)."
...
def crearPDF(arrPor, desc, cant, final, descr, nombre):
    ...
    #ancho = 612
    #alto = 792
    c=canvas.Canvas("Envio/"+nombre+".pdf", pagesize = letter)
    #cabecera
    c.setFont("Helvetica-Oblique",10)
    c.drawImage("logo.png",10,700,width=90,height=90)
    ...
    c.drawString(220,600,"Alineamientos")
    ...
    c.drawImage("logo.png",200,80,width=200,height=200)
...
def Mejores(cant, secInicial, descripciones, secuencias, arrayScore, matriz, gap_open, scoremax, mail, sec_id, final, nombre): # cant = la cantidad de secuencias $
    ...
    for s in range(0,cant):
        ...
        print aln_entrada+'\n\n'+aln_basemay
    """
    fa = open("Envio/"+nombre+".fasta", "w")
    fa.close()
    fa = open("Envio/"+nombre+".fasta", "a")
    for s in range(0,cant):
        secBase = secuencias[s]
        secIni = secInicial[s]
        arrSco = arrayScore[s]
